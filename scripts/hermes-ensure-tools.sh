...
# Starts the services Hermes commonly needs for SRC/pentest workflows when safe.
# Usage:
#   hermes-ensure-tools.sh --burp --hexstrike --gateway
#   hermes-ensure-tools.sh --all
#   hermes-ensure-tools.sh --status
set -u
...
start_hexstrike(){
  if port_open 8888; then
    ...
    if [ -f /root/hexstrike-ai/hexstrike_server.py ]; then
...
start_burp(){
  ...
  if port_open 8080; then
    ...
    if exists burpsuite; then
      ...
      for i in $(seq 1 "$WAIT_SECS"); do
        ...
        # handle common startup dialogs if xdotool is available
  ...
  if port_open 8080; then
    log "Burp proxy ready"
    # Try to turn intercept off without failing hard.
    if exists xdotool; then
      WID=$(DISPLAY="$DISPLAY_VALUE" xdotool search --name 'Burp Suite Community Edition\|Burp Suite' 2>/dev/null | head -1 || true)
      if [ -n "${WID:-}" ]; then
        DISPLAY="$DISPLAY_VALUE" xdotool windowactivate "$WID" >/dev/null 2>&1 || true
        ...
        sleep 0.3
        DISPLAY="$DISPLAY_VALUE" xdotool key ctrl+T >/dev/null 2>&1 || true
      fi
    fi
  else
    log "Burp proxy not ready after ${WAIT_SECS}s"
    return 1
  fi
}
